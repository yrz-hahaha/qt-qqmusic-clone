...
当然这种是最粗暴的办法，还有两种更加优雅的办法，在 Qt Creator 中直接右键 Qt 项目，然后执行以下操作：

**方法一：Clean → qmake → Build**
- **清除（Clean）**：本质上是执行 `make clean`。它会根据 Makefile 中定义的规则，物理删除所有中间产物（如 `.o`/`.obj` 目标文件、`ui_*.h` 头文件、`moc_*.cpp` 元对象文件等）。
- **执行 qmake（Run qmake）**：关键步骤。运行 `qmake` 工具重新扫描 `.pro` 项目描述文件，并根据最新的文件依赖关系生成一套全新的 **Makefile** 文件（编译规则）。
- **构建（Build）**：相当于执行 `make`。编译器会严格按照新生成的 Makefile 规则，从零开始将源代码编译并链接为最终的可执行程序。

**方法二：直接点击“重新构建（Rebuild）”**
- **重新构建 (Rebuild)**：这是“清理 + 构建”的自动化整合方案。相当于连续执行 `make clean` + `make`。并且重新构建通常**不会**主动触发 `qmake` ，也就是说重新构建通常**不会**主动去生成新的 Makefile 文件，它默认你之前的 Makefile 文件还是有效的。因此，如果你修改了 `.pro` 文件或增删了图片资源，单纯点击“重新构建”可能无法生效。

如果这两种方案解决不了就直接尝试暴力删除构建目录。